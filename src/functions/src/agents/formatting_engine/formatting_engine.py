...
class RuleBasedFormatter:
    ...
    def process(self, doc: Document, template_rules: dict) -> Document:
        ...
        """
        self._apply_base_style(doc, template_rules)
        ...
        # Post-pass: apply font explicitly to all runs and bold Q-level numbers
        self._apply_font_pass(doc, template_rules)
        self._bold_question_numbers(doc)
        self._ensure_question_spacing(doc)
        return doc
    ...
    def _apply_font_pass(self, doc: Document, template_rules: dict) -> None:
        """
        Apply base font name and size explicitly to every run in the document.

        Run-level formatting overrides style-level; we must set it here so
        the template font is applied even when the draft has per-run overrides.
        Skips paragraphs that contain math (m:oMath) runs.
        """
        base = template_rules.get("base_font") or {}
        font_name = base.get("name")
        font_size_pt = base.get("size_pt")
        if not font_name and not font_size_pt:
            return
        for para in doc.paragraphs:
            if self._contains_math(para):
                continue
    ...
    def _bold_question_numbers(self, doc: Document) -> None:
        """
        Bold every paragraph whose text begins with a Q-level question number
        (e.g. ``Q1.`` or ``Q12.``).

        This makes question numbers visually distinct without changing content.
        Sub-questions ``(a)`` and ``(i)`` are left as-is (not bolded).
        Skips math paragraphs.
        """
        for para in doc.paragraphs:
            if self._contains_math(para):
                continue
    ...
    def _ensure_question_spacing(self, doc: Document) -> None:
        """
        Ensure there is visible space before each Q-level paragraph.

        Adds 12 pt space-before to Q-level lines so questions are clearly
        separated even in dense drafts with no blank lines.
        """
        for para in doc.paragraphs:
            if self._contains_math(para):
                continue
            if re.match(r"^Q\d+\.", para.text.lstrip()):
                try:
                    pf = para.paragraph_format
                    # Only increase — don't reduce existing larger spacing
                    current = pf.space_before
                    if current is None or int(current) < 120000:  # 120000 EMU = ~9.5pt
                        pf.space_before = Pt(12)
                except Exception:
                    pass

    def _apply_base_style(self, doc: Document, template_rules: dict) -> None:
        """
        Apply base typography/spacing derived from the template docx.

        This targets the "Normal" style and (lightly) normalizes paragraph spacing.
        """
        base = template_rules.get("base_font") or {}
        normal_pf = template_rules.get("normal_paragraph_format") or {}
        ...
        try:
            if base.get("name"):
                normal.font.name = base["name"]
            if base.get("size_pt"):
                normal.font.size = Pt(float(base["size_pt"]))
        except Exception:
            pass

        # Paragraph spacing / line spacing on Normal style
        try:
            pf = normal.paragraph_format
            sb = normal_pf.get("space_before_pt")
            sa = normal_pf.get("space_after_pt")
            if sb is not None:
                pf.space_before = Pt(float(sb))
            if sa is not None:
                pf.space_after = Pt(float(sa))
            if normal_pf.get("line_spacing") is not None:
                pf.line_spacing = normal_pf.get("line_spacing")
        except Exception:
            pass
