...
class FileHandlerAgent:
    ...
    async def get_template_from_vectordb(self, query: str) -> dict:
        ...
        # upload_template.py stores template_rules as a JSON string for a simple schema
        if isinstance(template_rules, str):
            try:
                import json

                template_rules = json.loads(template_rules)
            except Exception:
                template_rules = {}
    ...
    async def save_outputs(
        ...
        def _upload_and_sas(blob_name: str, data: bytes, content_type: str) -> str:
            ...
            container_client.upload_blob(
                ...
                data,
                overwrite=True,
                content_settings=_CS(content_type=content_type),
            )
            sas_token = generate_blob_sas(
                account_name=service_client.account_name,
