...
# ── Default SUC formatting rules (used when Azure AI Search is unavailable) ──

_DEFAULT_SUC_RULES: dict = {
    "header_text": "SOUTHERN UNIVERSITY COLLEGE",
    "footer_text": "Page",
    "margin_cm": {"top": 2.5, "bottom": 2.5, "left": 3.0, "right": 2.5},
    "base_font": {"name": "Times New Roman", "size_pt": 12.0},
...
class CoordinatorAgent:
    ...
    async def process_job(
        self, job_id: str, user_id: str, file_url: str, template_url: str = ""
    ) -> dict:
        """
        ...
        if getattr(self, "team_leader", None) is not None:
            try:
                # SK path currently doesn't accept a template_url override
                if template_url:
                    return await self._manual_chain(job_id, user_id, file_url, template_url=template_url)
    ...
    async def _manual_chain(
        ...
        try:
            if template_url:
                template_doc = await self.file_handler.download_from_blob(template_url)
                from src.utils.template_rules_from_docx import extract_template_rules_from_docx
